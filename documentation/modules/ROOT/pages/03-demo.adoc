...
Let's interact with the chatbot. Run the following command to start the chatbot, since we are ilab, we will need another machine with ilab installed, or as in this case, we will use the same machine to interact with the chatbot, the idea here is to simulate the interaction between the user and the chatbot thru a service exposed by the public Skupper.

Run this command in a terminal where the virtual environment is activated, in our case ~/instructlab directory:
